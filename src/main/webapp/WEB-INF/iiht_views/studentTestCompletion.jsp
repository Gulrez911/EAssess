...
.container {
		...
		<div class="container" id="dom">
			<div class="testinformation">
				<div class="text-center">
					...
					<h1
						style="color: #7ac142; text-align: center; margin-top: 0; font-weight: bold;">${studentTestForm.firstName}
						${studentTestForm.lastName} - You have completed the test</h1>
				</div>
				<h3 style="text-align: center;">
					${studentTestForm.emailId.replaceAll(".+-", "").replaceAll("]","")}
					|||| ${studentTestForm.noOfAttempts} You completed the test in
					<%-- 										${studentTestForm.noOfAttempts == null || studentTestForm.noOfAttempts.trim().length() == 0?"1":studentTestForm.noOfAttempts} --%>
					<%-- 										${studentTestForm.noOfAttempts == null || studentTestForm.emailId.replaceAll(".+-", "").replaceAll("]","")==0?"1":studentTestForm.emailId.replaceAll(".+-", "").replaceAll("]","")} --%>
					<c:choose>
						<c:when test="${studentTestForm.noOfAttempts==null}">
						1
						</c:when>
						<c:otherwise>
							${studentTestForm.emailId.replaceAll(".+-", "").replaceAll("]","")}
					...
					<c:choose>
						<c:when
							test="${studentTestForm.noOfAttempts==null}">
						attempt
						</c:when>
						<c:otherwise>
				attempts
					...
					<%-- 					<%!int data = 50;%> --%>
					<%-- 										${studentTestForm.noOfAttempts == null || studentTestForm.emailId.replaceAll(".+-", "").replaceAll("]","") == 0?"attempt":"attempts"} --%>

				</h3>
				<h3 style="text-align: center; padding-bottom: 20px;">Your
					results have been shared by email to the Test Administrator -
				...
				<c:if test="${showResults}">
					<div class="row">
						...
						<div class="resultinformation">
							<h3>
								<i class="fa fa-bar-chart-o"></i> Result
							</h3>
						</div>
					...
					<h3
						style="font-weight: bold; background-color: rgb(0, 108, 255); padding: 10px; color: #fff; margin-bottom: 0;">Topic
						Wise Performance - See below</h3>
					...
					<div class="m-b-35">
						<c:if test="${showTraits}">
							<h3
								style="font-weight: bold; background-color: rgb(0, 108, 255); padding: 10px; color: #fff; margin-bottom: 0;">Some
								Learner Traits</h3>
							<table class="table" style="border: 1px solid #dddddd;">
					...
					<c:forEach var="ins" items="${codingInstances}">
						<div class="questionslist">
							<p>${ins.questionMapper.question.questionText}</p>
							<table class="table">
								<thead>
									<tr>
	...
	<script>
		function download() {
			domtoimage.toBlob(document.getElementById('dom')).then(
					function(blob) {
